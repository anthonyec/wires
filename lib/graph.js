...
function Graph() {
  return {
    ...
    executeComponent: function(uid, props) {
      ...
      const outletPropsFromComponent = this.components[uid].executor(propsToPassIn);
    ...
    getConnectionAsString: function(connection, ignoreStart) {
    ...
    start: function() {
      const toppies = this.getTopComponents();
      logger('start:', toppies);
      toppies.forEach((component) => component.execute());
    },
